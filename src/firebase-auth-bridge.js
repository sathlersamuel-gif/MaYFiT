...
function notice(form,text){
  ...
  if(!box){
    ...
    form.insertBefore(box,form.querySelector('button.primary'));
  }
...
function errorText(e){
  const map={
    ...
    'permission-denied':'O banco ainda está bloqueado pelas regras do Firestore.'
  };
...
async function ensureProfile(user,name=''){
  ...
  if(email===ADMIN_EMAIL){
    const fallback=adminProfile(user);
    try{
      ...
      await ref.set({
        uid:user.uid,
        email,
        fullName:name||data.fullName||user.displayName||fallback.name,
        role:'admin',
        status:'approved',
        updatedAt:firebase.firestore.FieldValue.serverTimestamp(),
        ...(snap.exists?{}:{createdAt:firebase.firestore.FieldValue.serverTimestamp()})
      },{merge:true});
      return {...fallback,name:name||data.fullName||fallback.name};
    }catch(error){
      if(error?.code==='permission-denied'){
        console.warn('MaYFiT: Firestore bloqueado; administrador liberado por autenticação Firebase.');
        return fallback;
      }
      throw error;
    }
  ...
  if(!snap.exists){
    await ref.set({
      uid:user.uid,
      email,
      fullName:name||user.displayName||email.split('@')[0],
      role:'student',
      status:'pending',
  ...
  return {
    id:user.uid,
    name:data.fullName||user.displayName||email.split('@')[0],
    email:user.email,
...
async function reset(form){
  ...
  const email=prompt('Digite o e-mail cadastrado:',typed);
  if(!email?.trim())return;
  try{
    await auth.sendPasswordResetEmail(email.trim());
    alert('Link para criar uma nova senha enviado ao seu e-mail. Verifique também o spam.');
  }catch(e){alert(errorText(e));}
}

function enhance(){
  const form=document.querySelector('.login-card');
  if(!form||form.dataset.firebaseReady==='1')return;
  form.dataset.firebaseReady='1';
  const inputs=form.querySelectorAll('input');
  if(inputs[0]){inputs[0].value='';inputs[0].placeholder='seuemail@exemplo.com';}
  if(inputs[1])inputs[1].value='';
  const demo=form.querySelector('.demo-switch');
  if(demo){
    demo.textContent='Criar nova conta';
    demo.onclick=e=>{e.preventDefault();e.stopPropagation();signup(form);};
  }
  const forgot=document.createElement('button');
  forgot.type='button';
  forgot.textContent='Esqueci minha senha';
  forgot.style.cssText='display:block;width:100%;margin:10px 0 2px;padding:8px;border:0;background:transparent;color:#8df20b;font:800 14px system-ui;text-decoration:underline';
  forgot.onclick=e=>{e.preventDefault();e.stopPropagation();reset(form);};
  const label=inputs[1]?.closest('label');
  if(label)label.insertAdjacentElement('afterend',forgot);
