...
function writeStore(data){const seen=new Set();const exercises=Array.isArray(data?.exercises)?data.exercises.filter(item=>{const type=clean(item?.type);if(!type||seen.has(type))return false;seen.add(type);return true}):[];localStorage.setItem(STORE,JSON.stringify({...data,exercises}));window.dispatchEvent(new Event('mayfit-store-updated'))}
...
function persistAdminNames(){const custom=readCustom();const store=readStore();if(!store||!Array.isArray(store.exercises))return;let changed=false;const exercises=store.exercises.map(item=>{const name=clean(custom[String(item.type)]);if(name&&name!==item.name){changed=true;return{...item,name}}return item});if(changed)writeStore({...store,exercises})}
