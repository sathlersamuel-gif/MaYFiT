...
#mayfit-students .ms-card.pending{border-color:#9b7a24;box-shadow:inset 3px 0 0 #e7b931}
...
#mayfit-students .ms-badge.pending{background:#4b3b12;color:#ffd96a}
...
async function loadStudents(root){
  ...
  root._students=data||[];
  const pending=root._students.filter(x=>x.status==='pending').length;
  msg.textContent=pending?`${root._students.length} aluno(s) • ${pending} aguardando aprovação`:`${root._students.length} aluno(s) encontrado(s)`;
  render(root);
}
...
function render(root){
  ...
  list.innerHTML=data.map(x=>{
    const mainAction=x.status==='pending'
      ?'<button class="ms-approve" data-action="approve">Aprovar aluno</button>'
      :`<button class="ms-secondary" data-action="toggle">${x.status==='blocked'?'Desbloquear':'Bloquear'}</button>`;
    return `<article class="ms-card ${esc(x.status)}" data-id="${esc(x.id)}"><div class="ms-row"><div><div class="ms-name">${esc(x.full_name||'Aluno sem nome')}</div><div class="ms-email">${esc(x.email||'E-mail não informado')}</div></div><span class="ms-badge ${esc(x.status)}">${statusLabel(x.status)}</span></div><div class="ms-card-actions">${mainAction}<button class="ms-secondary" data-action="edit">Editar</button><button class="ms-danger" data-action="delete">Excluir</button></div></article>`;
  }).join('');
}
...
async function handleCard(root,button){
  ...
  if(action==='approve'){
    const {error}=await supabase.from('profiles').update({status:'active',role:'student'}).eq('id',id);
    if(error){button.disabled=false;return alert('Não foi possível aprovar: '+error.message)}
    alert('Aluno aprovado com sucesso.');
  }
  if(action==='edit'){
    const name=prompt('Nome completo:',student.full_name||''); if(!name?.trim()){button.disabled=false;return}
  ...
  if(action==='delete'){
    if(!confirm(`Excluir o aluno ${student.full_name||''}?`)){button.disabled=false;return}
    const {error}=await supabase.from('profiles').delete().eq('id',id);
    if(error){button.disabled=false;return alert('Não foi possível excluir: '+error.message)}
  }
  loadStudents(root);
}
