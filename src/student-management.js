...
async function loadStudents(root){
  ...
  try{
    ...
    const pendingIds=root._students.filter(profile=>profile.status==='pending').map(profile=>profile.id);
    if(pendingIds.length){
      const {error:activateError}=await supabase.from('profiles').update({status:'active'}).in('id',pendingIds);
      if(!activateError)root._students=root._students.map(profile=>profile.status==='pending'?{...profile,status:'active'}:profile);
    }
...
function render(root){
  ...
  list.innerHTML=data.map(x=>{
    ...
    const toggle=`<button class="ms-secondary" data-action="toggle">${status==='blocked'?'Desbloquear':'Bloquear'}</button>`;
    const open=status==='active'?'<button class="ms-primary" data-action="open">Abrir área do aluno</button>':'';
    return `<article class="ms-card ${status}" data-id="${esc(x.id)}"><div class="ms-row"><div><div class="ms-name">${esc(x.full_name||'Aluno sem nome')}</div></div><span class="ms-badge ${status}">${statusLabel(status)}</span></div><div class="ms-card-actions">${open}<button class="ms-secondary" data-action="edit">Editar</button>${toggle}<button class="ms-danger" data-action="delete">Excluir</button></div></article>`;
  }).join('');
}
