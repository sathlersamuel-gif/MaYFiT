...
function directTextNodes(root){
  return [...root.childNodes].filter(node=>node.nodeType===Node.TEXT_NODE);
}

function adjustStudentEvolutionCard(){
  if(currentUser()?.role!=='student')return;
  const candidates=[...document.querySelectorAll('button,article,div')].filter(element=>{
    const text=(element.textContent||'').trim();
    return /treinos salvos/i.test(text)&&!element.closest('.mayfit-history-overlay');
  });
  const card=candidates.sort((a,b)=>a.getBoundingClientRect().width-b.getBoundingClientRect().width)[0];
  if(!card||card.dataset.evolutionCardAdjusted==='true')return;

  const numeric=[...card.querySelectorAll('strong,b,span,div')].find(element=>/^\s*\d+\s*$/.test(element.textContent||''));
  if(numeric)numeric.style.display='none';

  const walker=document.createTreeWalker(card,NodeFilter.SHOW_TEXT);
  const nodes=[];
  while(walker.nextNode())nodes.push(walker.currentNode);
  const label=nodes.find(node=>/treinos salvos/i.test(node.nodeValue||''));
  if(label)label.nodeValue=(label.nodeValue||'').replace(/treinos salvos/gi,'Evolução de cargas');
...
function warmVisibleExercisePhotos(){
  ...
  warmTimer=setTimeout(()=>{
    const urls=[...document.querySelectorAll('#mse-modal img.mse-thumb,.exercise-picker img.mayfit-picker-thumb')]
...
document.addEventListener('click',event=>{
  if(event.target.closest('[data-action="add"], [data-action="remove"], #mayfit-student-exercises button'))setTimeout(warmVisibleExercisePhotos,250);
},true);
apply();
