...
let mounted=false,loading=false,allExercises=[],selected=new Set();
...
#mse-modal h2{margin:0;font-size:21px}.mse-close{width:38px;height:38px;padding:0!important;border:1px solid #385442!important;border-radius:12px!important;background:#17231b!important;color:#fff!important;font-size:22px!important}
...
#mse-modal .mse-item.used{opacity:.52}.mse-footer{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:13px 15px;border-top:1px solid #263d2d;background:#0d1711}.mse-count{color:#a9b8af;font-size:13px}.mse-add{border:0;border-radius:12px;padding:12px 16px;background:#78d532;color:#07110c;font-weight:950}.mse-add:disabled{opacity:.5}
.mse-image-zoom{position:fixed;inset:0;z-index:100001;display:grid;place-items:center;padding:20px;background:rgba(0,0,0,.92)}.mse-image-zoom img{max-width:min(920px,96vw);max-height:86vh;object-fit:contain;border-radius:16px}.mse-image-zoom button{position:fixed;top:max(18px,env(safe-area-inset-top));right:18px;width:44px;height:44px;border:1px solid #49664f;border-radius:14px;background:#132018;color:#fff;font-size:26px}
...
function openImage(src,name){document.querySelector('.mse-image-zoom')?.remove();const zoom=document.createElement('div');zoom.className='mse-image-zoom';zoom.innerHTML=`<button type="button">×</button><img src="${esc(src)}" alt="${esc(name)}">`;zoom.querySelector('button').onclick=()=>zoom.remove();zoom.onclick=e=>{if(e.target===zoom)zoom.remove()};document.body.appendChild(zoom)}
...
function renderList(modal){
  ...
  modal.querySelector('.mse-list').innerHTML=filtered.map(item=>{
    const exists=used.has(item.id),image=imageFor(item);
  ...
  modal.querySelector('.mse-count').textContent=`${selected.size} selecionado(s) • ${filtered.length} exercício(s)`;
  modal.querySelector('.mse-add').disabled=!selected.size;
}

async function loadCatalog(modal){
  if(allExercises.length){renderList(modal);return}if(loading)return;loading=true;
  modal.querySelector('.mse-list').innerHTML='<div style="padding:20px;text-align:center;color:#96a49a">Carregando exercícios...</div>';
  try{const response=await fetch(DB,{cache:'no-store'});if(!response.ok)throw new Error('Falha ao carregar catálogo');const data=await response.json();allExercises=(Array.isArray(data)?data:[]).map(item=>({id:item.id,name:item.name,image:Array.isArray(item.images)?item.images[0]:''})).filter(item=>item.id&&item.name).sort((a,b)=>a.name.localeCompare(b.name,'pt-BR'));renderList(modal)}catch(error){modal.querySelector('.mse-list').innerHTML=`<div style="padding:20px;text-align:center;color:#ffb4b4">${esc(error.message)}. Tente novamente.</div>`}finally{loading=false}
}

function addSelected(){
  const store=readStore();if(!store||!Array.isArray(store.exercises)){alert('Não foi possível acessar os treinos salvos.');return}
  const used=new Set(store.exercises.map(item=>item.type));let nextId=Math.max(0,...store.exercises.map(item=>Number(item.id)||0));
  const additions=allExercises.filter(item=>selected.has(item.id)&&!used.has(item.id)).map(item=>({id:++nextId,type:item.id,name:item.name,sets:3,reps:12,load:0,previousLoad:0,rest:60,tip:'Execute o movimento com controle e postura correta.'}));
  if(!additions.length){alert('Os exercícios selecionados já foram adicionados.');return}
  writeStore({...store,exercises:[...store.exercises,...additions]});alert(`${additions.length} exercício(s) adicionado(s) com sucesso.`);location.reload();
}

function openModal(){
  selected=new Set();const modal=document.createElement('div');modal.id='mse-modal';
  modal.innerHTML='<div class="mse-card"><div class="mse-top"><div><h2>Adicionar exercícios</h2><div style="color:#9cac9f;font-size:13px;margin-top:4px">Mesmo catálogo completo do administrador.</div></div><button class="mse-close" type="button">×</button></div><input class="mse-search" placeholder="Pesquisar exercício"><div class="mse-list"></div><div class="mse-footer"><span class="mse-count">0 selecionado(s)</span><button class="mse-add" type="button" disabled>Adicionar selecionados</button></div></div>';
  document.body.appendChild(modal);const close=()=>modal.remove();modal.querySelector('.mse-close').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});modal.querySelector('.mse-search').oninput=()=>renderList(modal);
  modal.querySelector('.mse-list').addEventListener('click',event=>{const image=event.target.closest('.mse-thumb[data-image]');if(!image)return;event.preventDefault();event.stopPropagation();openImage(image.dataset.image,image.alt)},true);
  modal.querySelector('.mse-list').addEventListener('change',event=>{const input=event.target.closest('input[type="checkbox"]');if(!input||input.disabled)return;input.checked?selected.add(input.value):selected.delete(input.value);renderList(modal)});
  modal.querySelector('.mse-add').onclick=addSelected;loadCatalog(modal);
}

function mount(){
  if(mounted||currentUser()?.role!=='student')return false;const main=document.querySelector('.app main');if(!main||document.querySelector('.workout-screen'))return false;
  ...
  const section=document.createElement('section');section.id='mayfit-student-exercises';section.innerHTML='<div class="mse-head"><div><h2>Meus exercícios</h2><p>Adicione novos exercícios ao seu treino.</p></div><button type="button">Adicionar exercícios</button></div>';section.querySelector('button').onclick=openModal;main.prepend(section);mounted=true;return true;
}
