...
async function installDeleteEvaluationButtons(modal) {
  if (modal.querySelector("[data-delete-evaluation]")) return;
  ...
  entries.forEach((entry, index) => {
    ...
    button.style.cssText =
      "width:100%;margin-top:12px;padding:10px 12px;border:1px solid #753737;border-radius:11px;background:#3a1717;color:#ffb3b3;font-weight:900";

    button.onclick = async () => {
      if (
        !confirm(
          "Excluir esta avaliação completa? As medidas, observações e todas as fotos serão apagadas definitivamente.",
        )
      )
        return;
...
function scan() {
  ...
  document
    ...
    .forEach((modal) => installDeleteEvaluationButtons(modal));
}
