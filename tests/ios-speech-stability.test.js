...
test("iPhone estabiliza a voz sem alterar os efeitos do treino", async () => {
  const [stability, notifications, mobileSounds] = await Promise.all([
    readFile("src/lib/ios-speech-stability.js", "utf8"),
    readFile("src/lib/workout-timer-notifications.js", "utf8"),
    readFile("src/mobile-workout-sounds.js", "utf8"),
  ]);

  assert.match(notifications, /import "\.\/ios-speech-stability\.js"/);
  ...
  assert.match(stability, /setTimeout\(\(\) => \{/);
